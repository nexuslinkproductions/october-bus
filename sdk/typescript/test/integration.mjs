...
try {
  ...
  const plannerRegistration = await owner.registerAgent({ id: 'planner', displayName: 'Planner' })
  const reviewerRegistration = await owner.registerAgent({
    id: 'reviewer',
    displayName: 'Reviewer',
    connectTo: ['planner']
  })
  const planner = new OctoberBusClient(run.address, plannerRegistration.agentToken)
  const reviewer = new OctoberBusClient(run.address, reviewerRegistration.agentToken)
  const peers = await planner.listPeers()
  assert.equal(peers.length, 1)
  assert.equal(peers[0].id, 'reviewer')
  const message = { to: 'reviewer', mode: 'request', body: 'Review this', idempotencyKey: 'integration-review' }
